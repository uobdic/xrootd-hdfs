...
void ChecksumState::Update(const unsigned char* buffer, size_t bsize)
{
    m_offset += bsize;
    ...
    if (m_digests & ChecksumManager::CVMFS)
    {
        ...
        while (total_bytes >= CVMFS_CHUNK_SIZE)
        { // There are at least CVMFS_CHUNK_SIZE bytes to write!
...
int ChecksumManager::Set(const char* pfn, const ChecksumState& state) const
{
...
int ChecksumManager::Calc(const char* pfn, XrdCksData& cks, int do_set)
{
    int digests = 0;
    int return_digest = 0;
    if (do_set)
