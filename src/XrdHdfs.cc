...
/*                                  O p e n d i r                             */
/******************************************************************************/
XrdHdfsDirectory::XrdHdfsDirectory(const char* tid)
    : XrdOssDF()
    , fs(NULL)
    , dh(NULL)
    , numEntries(0)
...
/******************************************************************************/
XrdHdfsFile::XrdHdfsFile(const char* user)
    : XrdOssDF()
    , m_fs(NULL)
    , fh(NULL)
    , fname(NULL)
...
const char* XrdHdfsSys::getVersion()
{
    return "@devel@";
}

void XrdHdfsSys::Say(char const* msg, char const* x, char const* y, char const* z)
{
    eDest->Say(msg, x, y, z);
}

char* XrdHdfsSys::GetRealPath(const char* path)
{
    char* fname;
    if (the_N2N)
    {
...
{
    if (the_N2N)
        return -(the_N2N->lfn2pfn(oldp, newp, blen));
    if ((int)strlen(oldp) >= blen)
        return -ENAMETOOLONG;
    strcpy(newp, oldp);
    return 0;
}
...
const char* XrdHdfsSys::Lfn2Pfn(const char* oldp, char* newp, int blen, int& rc)
{
    ...
    if ((rc = -(the_N2N->lfn2pfn(oldp, newp, blen))))
        return 0;
    return newp;
}
...
int XrdHdfsSys::Chmod(const char* req_fname, mode_t mode, XrdOucEnv* envp)
{
...
int XrdHdfsSys::Mkdir(const char* req_path, mode_t mode, int mkpath,
    XrdOucEnv* envp)
{
    ...
    if (MKDIR_PERFORM_CHMOD && mode)
    {
        ...
        if (NULL == fileInfo)
        {
            retc = XrdHdfsSys::Emsg(epname, error, errno ? errno : ENOENT, "stat", path);
            goto cleanup;
        }
        ...
        {
            retc = XrdHdfsSys::Emsg(epname, error, errno ? errno : EIO, "chmod", path);
            goto cleanup;
        }
    }
...
int XrdHdfsSys::Remdir(const char* req_path, int /*opts*/, XrdOucEnv* envp)
{
...
int XrdHdfsSys::Rename(const char* req_src, const char* req_dest,
...
int XrdHdfsSys::Truncate(const char* req_path, long long unsigned int newsize, XrdOucEnv* envp)
{
...
int XrdHdfsSys::Unlink(const char* req_path, int /*opts*/, XrdOucEnv* envp)
{
...
int XrdHdfsSys::Create(const char* tident, const char* req_path, mode_t mode,
